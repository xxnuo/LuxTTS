...
class LuxTTS:
    ...
    def encode_prompt(self, prompt_audio, duration=None, rms=0.001, prompt_text=None):
        prompt_tokens, prompt_features_lens, prompt_features, prompt_rms = process_audio(prompt_audio, self.transcriber, self.tokenizer, self.feature_extractor, self.device, target_rms=rms, duration=duration, prompt_text=prompt_text)
